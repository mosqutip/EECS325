(defun bst-elements (bst &optional lst)
  (when bst
    (setf lst (cons lst (node-elt bst)))
    (print lst)
    (bst-elements (node-r bst) lst)
    (bst-elements (node-l bst) lst)
    lst))

(defun bst-elements (bst)
  (order-bst bst nil))

(defun order-bst (bst bst-child)
  (cond ((null bst) nil)
        ((numberp bst)
         (cons bst-child bst)
         (t (let ((e (node-elt bst))
                  (l (node-l bst))
                  (r (node-r bst)))
              (cons e (order-bst r (order-bst l bst-child))))))))
